...
<head>
    ...
    <title>Agendar Horário - {{ $settings['company_name'] ?? config('app.name', 'BarberFlow') }}</title>
    
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    
    <!-- FontAwesome para ícones públicos -->
...
<body>
    ...
    <header class="hero-section text-center">
        <div class="container">
            @if(isset($settings['company_logo']) && $settings['company_logo'])
                <img src="{{ asset($settings['company_logo']) }}" alt="Logo" class="img-fluid rounded-circle shadow mb-3" style="width: 100px; height: 100px; object-fit: cover;">
            @else
                <i class="fa-solid fa-scissors fa-4x mb-3 text-primary"></i>
            @endif
            <h1 class="fw-bold">{{ $settings['company_name'] ?? 'Barbearia Premium' }}</h1>
            <p class="lead opacity-75">Agende seu horário de forma rápida e prática</p>
        </div>
    </header>
                ...
                <div class="card booking-card bg-white p-4 p-md-5 h-100">
                    ...
                    @if(session('success'))
                        <div class="alert alert-success alert-dismissible fade show rounded-4 p-4 text-center" role="alert">
                            ...
                            <h4 class="alert-heading fw-bold">Tudo Certo!</h4>
                    ...
                    <form action="{{ route('public.booking.store') }}" method="POST">
                        ...
                        <h5 class="fw-bold text-primary mb-3"><i class="fa-solid fa-user me-2"></i>Seus Dados</h5>
                        <div class="row mb-4">
                            <div class="col-md-6 mb-3 mb-md-0">
                                <label class="form-label fw-semibold">Como podemos te chamar? <span class="text-danger">*</span></label>
                                <input type="text" name="cliente_nome" class="form-control form-control-lg bg-light" placeholder="Seu Nome Completo" value="{{ old('cliente_nome') }}" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Seu melhor WhatsApp <span class="text-danger">*</span></label>
                                <input type="tel" name="cliente_whatsapp" class="form-control form-control-lg bg-light" placeholder="(DD) 90000-0000" value="{{ old('cliente_whatsapp') }}" required>
                            </div>
                        </div>

                        <h5 class="fw-bold text-primary mb-3 mt-5"><i class="fa-solid fa-cut me-2"></i>Preferências</h5>
                        <div class="row mb-4">
                            <div class="col-md-6 mb-3 mb-md-0">
                                <label class="form-label fw-semibold">Escolha o Serviço <span class="text-danger">*</span></label>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="barber_id" id="barber_select" class="form-select form-select-lg bg-light" required>
                                    <option value="" disabled selected>Quem vai te atender?</option>
                        ...
                        <h5 class="fw-bold text-primary mb-3 mt-5"><i class="fa-regular fa-clock me-2"></i>Quando?</h5>
                        <div class="row mb-5">
                            <div class="col-md-6 mb-3 mb-md-0">
                                <label class="form-label fw-semibold">Data <span class="text-danger">*</span></label>
                                <input type="date" name="data" id="data_input" class="form-control form-control-lg bg-light" value="{{ old('data', date('Y-m-d')) }}" required>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-semibold">Horário <span class="text-danger">*</span></label>
                                <select name="hora" id="hora_select" class="form-select form-select-lg bg-light" required>
                                    <option value="" disabled selected>Preencha data e profissional...</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <h5 class="fw-bold text-primary mb-3 mt-5"><i class="fa-solid fa-shield-halved me-2"></i>Verificação Anti-Bot</h5>
                        <div class="row mb-4">
                            <div class="col-md-12">
                                <label class="form-label fw-semibold">Quanto é {{ $num1 }} + {{ $num2 }}? <span class="text-danger">*</span></label>
                                <input type="number" id="captcha_input" class="form-control form-control-lg bg-light" placeholder="Sua resposta" required>
                                <input type="hidden" id="captcha_answer" value="{{ $sum }}">
                            </div>
                        </div>

                        <div class="d-grid mt-4">
                            <button type="submit" id="submit_btn" class="btn btn-primary btn-lg fw-bold rounded-pill p-3 shadow-sm" disabled>
                                <i class="fa-solid fa-calendar-check me-2"></i> Finalizar Agendamento
                            </button>
                        </div>
                    </form>
    ...
    <footer class="text-center py-4 text-muted small">
        <p class="mb-0">
            ...
            @endif
            BarberFlow &copy; {{ date('Y') }} - Desenvolvido por Rafael Maciel
        </p>
        <div class="mt-2">
            <a href="{{ route('login') }}" class="text-decoration-none text-muted">Acesso Restrito</a>
        </div>
    </footer>

    <!-- SweetAlert via CDN para alerts caso o layout não tenha puxado -->
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Lógica de buscar horários disponíveis
            ...
            function fetchAvailableTimes() {
                ...
                fetch(`/api/available-times?barber_id=${barberId}&date=${date}`)
                    ...
                    .then(times => {
                        ...
                        // Atualiza status do botão baseado no captcha se mudar o horário
            ...
            // Fetch inicial caso old() tenha preenchido
