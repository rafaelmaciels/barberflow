...
<body>
    ...
    <div class="header">
        <h1>BarberFlow - Relatório Administrativo</h1>
        <p>
            Período: {{ \Carbon\Carbon::parse($request->start_date)->format('d/m/Y') }} a {{ \Carbon\Carbon::parse($request->end_date)->format('d/m/Y') }}
            <br>
            Tipo: 
            @if($type == 'appointments') Agendamentos
            @elseif($type == 'finance') Financeiro
            @elseif($type == 'services') Desempenho de Serviços
            @elseif($type == 'barbers') Desempenho de Barbeiros
            @endif
        </p>
    </div>
    ...
    @else
        <table>
                ...
                <tbody>
                    @foreach($data as $row)
                        <tr>
                            ...
                            <td>
                                @if($row->status == 'agendado') Agendado
                                @elseif($row->status == 'em_atendimento') Em Atendimento
                                @elseif($row->status == 'concluido') Concluído
                                @elseif($row->status == 'cancelado') Cancelado
                                @else Não Compareceu @endif
                            </td>
                        </tr>
                    @endforeach
                </tbody>
            @elseif($type == 'finance')
                ...
                <tbody>
                    @php $totalReceita = 0; $totalDespesa = 0; @endphp
                    @foreach($data as $row)
                        @php
                ...
                <tfoot>
                    <tr>
                        ...
                        <td class="text-end fw-bold {{ ($totalReceita - $totalDespesa) >= 0 ? 'text-success' : 'text-danger' }}">
    ...
    <div class="footer">
        Documento gerado pelo sistema BarberFlow em {{ date('d/m/Y H:i:s') }}
    </div>
</body>
</html>
